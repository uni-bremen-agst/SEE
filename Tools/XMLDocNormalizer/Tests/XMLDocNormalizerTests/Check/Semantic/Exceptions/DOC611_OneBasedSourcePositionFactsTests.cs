...
namespace XMLDocNormalizerTests.Check.Semantic.Exception
{
    ...
    {
        /// <summary>
        /// Ensures that one-based line and column values derived from a successful
        /// Roslyn line-span lookup do not propagate the guarded out-of-range
        /// exception from the receiving method.
        /// </summary>
        [Fact]
        ...
        """;
            ...
            INamedTypeSymbol argumentOutOfRangeException =
                run.GetRequiredType("System.ArgumentOutOfRangeException");

            IReadOnlyList<ExceptionFlowPath> paths =
                run.Result.GetExceptionPaths(argumentOutOfRangeException);

            Assert.DoesNotContain(
                paths,
                path => path.Steps.Any(
                    step =>
                        step.Kind == ExceptionFlowPathStepKind.MethodCall
                        && step.SymbolName.Contains("Create", StringComparison.Ordinal)));
        }
