    ...
    public static class EvaluationReportWriter
    {
        ...
        public static string CreateMarkdown(EvaluationReport report)
        {
                         ...
                         StringComparer.Ordinal))
            {
                ...
                builder.AppendLine($"- Signing: {candidate.Signing.TargetState}; key token `{candidate.Signing.PublicKeyToken}`; signature {candidate.Signing.StrongNameSignatureSize} bytes; fidelity {candidate.Signing.FidelityResult}");
                builder.AppendLine($"- Signing options: CryptoPublicKey {(candidate.Signing.ReconstructedCryptoPublicKey?.Length ?? 0) / 2} bytes; CryptoKeyFile `{candidate.Signing.CryptoKeyFile ?? "null"}`; CryptoKeyContainer `{candidate.Signing.CryptoKeyContainer ?? "null"}`; DelaySign `{candidate.Signing.DelaySign?.ToString() ?? "null"}`; PublicSign `{candidate.Signing.PublicSign}`; StrongNameProvider `{candidate.Signing.StrongNameProviderConfigured}`");
