...
if [ -n "$CI" ]; then
  ...
  CHANGED_ASSETS=$(git diff --name-only --diff-filter=AM origin/master -- Assets ':!Assets/Editor' ':!Assets/Native' ':!Assets/StreamingAssets' ':!Assets/csc.rsp*' ':!Assets/NuGet.config*' ':!Assets/packages.config*' ':!Assets/Resources' ':!Assets/Scenes' ':!Assets/SEE*' ':!Assets/XR' ':!Assets/Settings' | git check-attr --stdin filter | grep -v "filter: lfs" || exit 0)
