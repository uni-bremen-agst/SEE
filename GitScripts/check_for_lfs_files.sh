...
if [ -n "$CI" ]; then
    ...
    shopt -s extglob
    CHANGED_ASSETS=$(git diff --name-only --diff-filter=AM origin/master -- Assets/!(Editor|Native|StreamingAssets|Resources|SEE*)/ | git check-attr --stdin filter | grep -v "filter: lfs" || exit 0)
    shopt -u extglob
