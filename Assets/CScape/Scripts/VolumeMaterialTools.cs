...
using UnityEditor;
[ExecuteInEditMode]
public class VolumeMaterialTools : MonoBehaviour
{
...
/////
        Color[] colorArray = new Color[size * size * slices];
        Texture3D textureArray = new Texture3D(size, size, slices, TextureFormat.RGBA32, true);
        float r = 1.0f / (size - 1.0f);

        Color[] texColor = new Color [size *size * slices];
        for (int x = 0; x < slices; x++)
        {

                string filename = string.Format(filePattern, x + 1);
                Texture2D tex = (Texture2D)Resources.Load(filename);
                Texture2D scaleTex = scaled(tex, size, size, 0);
            Color[] texCol = scaleTex.GetPixels(0);

            for (int y = 0; y < size; y++) {
                texColor[x * size] = texCol[x];
            }
