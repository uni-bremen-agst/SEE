...
namespace SEE.Game.Drawable.ActionHelpers
{
    ...
    public static class ShapePointsCalculator
    {
        ...
        /// <summary>
        /// Calculates the positions for a rectangle with chamfered corners.
        /// This calculation uses the chamfered values a/10 and b/10.
        /// </summary>
        /// <param name="point">The middle point of the rectangle. It's the hit point on the surface.</param>
        /// <param name="a">Specifies the edge length of the a sides of the rectangle.</param>
        /// <param name="b">Specifies the edge length of the b sides of the rectangle.</param>
        /// <returns>The positions for the rectangle.</returns>
        public static Vector3[] RectangleChamferedCorner(Vector3 point, float a, float b)
        {
            return RectangleChamferedCorner(point, a, b, a/10, b/10);
        }
        ...
        /// <param name="a">Specifies the edge length of the a sides of the rectangle.</param>
        /// <param name="b">Specifies the edge length of the b sides of the rectangle.</param>
        /// <param name="chamferedA">The chamfered value for the a sides.</param>
        /// <param name="chamferedB">The chamfered value for the b sides.</param>
        /// <returns>The positions for the rectangle.</returns>
        public static Vector3[] RectangleChamferedCorner(Vector3 point, float a, float b, float chamferedA, float chamferedB)
        {
            Vector3 A = new Vector3(point.x - a / 2 + chamferedA, point.y - b / 2, 0) - ValueHolder.DistanceToDrawable;
            Vector3 B = new Vector3(A.x + a, A.y, 0) - ValueHolder.DistanceToDrawable;
            Vector3 C = new Vector3(B.x, B.y + b, 0) - ValueHolder.DistanceToDrawable;
            Vector3 D = new Vector3(A.x, A.y + b, 0) - ValueHolder.DistanceToDrawable;

            Vector3 Ax = new Vector3(A.x + chamferedA, A.y) - ValueHolder.DistanceToDrawable;
            Vector3 Ay = new Vector3(A.x, A.y + chamferedB, 0) - ValueHolder.DistanceToDrawable;

            Vector3 Bx = new Vector3(A.x + a - chamferedA, A.y, 0) - ValueHolder.DistanceToDrawable;
            Vector3 By = new Vector3(B.x, B.y + chamferedB, 0) - ValueHolder.DistanceToDrawable;

            Vector3 Cx = new Vector3(C.x - chamferedA, C.y, 0) - ValueHolder.DistanceToDrawable;
            Vector3 Cy = new Vector3(C.x, C.y - chamferedB, 0) - ValueHolder.DistanceToDrawable;

            Vector3 Dx = new Vector3(D.x + chamferedA, D.y, 0) - ValueHolder.DistanceToDrawable;
            Vector3 Dy = new Vector3(D.x, D.y - chamferedB, 0) - ValueHolder.DistanceToDrawable;

            return new Vector3[] { Ax, Bx, By, Cy, Cx, Dx, Dy, Ay, Ax };
        }

        public static Vector3[] RectangleWithRoundedCorner(Vector3 point, float a, float b, float? chameredA = null, float? chameredB = null)
        {
            float distanceA = chameredA != null? chameredA.Value : a / 10;
            float distanceB = chameredB != null? chameredB.Value : b / 10;

            float radius = distanceA > distanceB ? distanceB : distanceA;

            Vector3 A = new Vector3(point.x - a / 2 + distanceA, point.y - b / 2, 0) - ValueHolder.DistanceToDrawable;
            Vector3 B = new Vector3(A.x + a, A.y, 0) - ValueHolder.DistanceToDrawable;
            Vector3 C = new Vector3(B.x, B.y + b, 0) - ValueHolder.DistanceToDrawable;
            Vector3 D = new Vector3(A.x, A.y + b, 0) - ValueHolder.DistanceToDrawable;

            Vector3 Ax = new Vector3(A.x + distanceA, A.y) - ValueHolder.DistanceToDrawable;
            Vector3 Ay = new Vector3(A.x, A.y + distanceB, 0) - ValueHolder.DistanceToDrawable;
            Vector3[] cornerA = CalculateBezierCurve(Ay, Ax, -radius);//AddCornerPoints(A, 0.05f, 90);//DrawQuadraticBezierCurve(Ax, Ay, A);


            Vector3 Bx = new Vector3(A.x + a - distanceA, A.y, 0) - ValueHolder.DistanceToDrawable;
            Vector3 By = new Vector3(B.x, B.y + distanceB, 0) - ValueHolder.DistanceToDrawable;
            Vector3[] cornerB = CalculateBezierCurve(Bx, By, -radius);//AddCornerPoints(B, 0.01f, -90);//DrawQuadraticBezierCurve(Bx, By, B);


            Vector3 Cx = new Vector3(C.x - distanceA, C.y, 0) - ValueHolder.DistanceToDrawable;
            Vector3 Cy = new Vector3(C.x, C.y - distanceB, 0) - ValueHolder.DistanceToDrawable;
            Vector3[] cornerC = CalculateBezierCurve(Cy, Cx, -radius);//AddCornerPoints(C, 0.05f, 270);//DrawQuadraticBezierCurve(Cx, Cy, C);


            Vector3 Dx = new Vector3(D.x + distanceA, D.y, 0) - ValueHolder.DistanceToDrawable;
            Vector3 Dy = new Vector3(D.x, D.y - distanceB, 0) - ValueHolder.DistanceToDrawable;
            Vector3[] cornerD = CalculateBezierCurve(Dx, Dy, -radius);//AddCornerPoints(D, 0.05f, 180);//DrawQuadraticBezierCurve(Dx, Dy, D);

            List<Vector3> positions = new() { Ax, Bx };
            positions.AddRange(cornerB);
            positions.Add(By);
            positions.Add(Cy);
            positions.AddRange(cornerC);
            positions.Add(Cx);
            positions.Add(Dx);
            positions.AddRange(cornerD);
            positions.Add(Dy);
            positions.Add(Ay);
            positions.AddRange(cornerA);
            positions.Add(Ax);
        ...
        static Vector3[] CalculateBezierCurve(Vector3 start, Vector3 end, float radius)
        {
            int cornerSegments = 20;
            // Berechne den Kontrollpunkt für die nach außen gewölbte Ecke
            Vector3 midPoint = (start + end) / 2;
            Vector3 direction = (end - start).normalized;
            Vector3 perpendicular = new Vector3(-direction.y, direction.x, direction.z); // 90 Grad Rotation
            Vector3 controlPoint = midPoint + perpendicular * radius;

            Vector3[] points = new Vector3[cornerSegments + 1];

            for (int i = 0; i <= cornerSegments; i++)
            {
                float t = i / (float)cornerSegments;
                points[i] = Mathf.Pow(1 - t, 2) * start + 2 * (1 - t) * t * controlPoint + Mathf.Pow(t, 2) * end;
            }

            return points;
        }

        private static Vector3[] AddCornerPoints(Vector3 center, float radius, float startAngle)
        {
            int segmentCount = 50;
            Vector3[] points = new Vector3[segmentCount];
            for (int i = 0; i < segmentCount; i++)
            {
                float angle = startAngle + (i * 90f / segmentCount);
                float rad = Mathf.Deg2Rad * angle;
                points[i] = new Vector3(center.x + Mathf.Cos(rad) * radius, center.y + Mathf.Sin(rad) * radius);
            }
            return points;
        }

        /*
        private static Vector3[] DrawQuadraticBezierCurve(Vector3 start, Vector3 end, Vector3 control)
        {
            int segmentCount = 50;
            Vector3[] corner = new Vector3[segmentCount];
            for (int i = 0; i < segmentCount; i++)
            {
                float t = i / segmentCount;
                Vector3 position = CalculateQuadraticBezierPoint(t, start, end, control);
                corner[i] = position;
            }
            return corner;
        }

        private static Vector3 CalculateQuadraticBezierPoint(float t, Vector3 start, Vector3 end, Vector3 control)
        {
