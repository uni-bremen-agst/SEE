...
namespace SEE.Game
{
    ...
    public class GraphRenderer
    {
        ...
        /// </summary>
        private readonly NodeFactory[] leafNodeFactories = new NodeFactory[(int)Node.Kind.Count];
        ...
        public void Apply(GameObject gameNode, GameObject itsParent, ILayoutNode layout)
        {
            ...
            if (node.IsLeaf())
            {
                NodeFactory leafNodeFactory = leafNodeFactories[(int)node.kind];
            ...
            if (node.IsLeaf())
            {
                leafNodeFactories[(int)node.kind].Rotate(gameNode, layout.Rotation);
            }
            else
            {
                innerNodeFactories[(int)node.kind].Rotate(gameNode, layout.Rotation);
            }
        ...
        {
            switch (settings.nodeLayoutSettings.kind)
            {
                case NodeLayoutKind.Manhattan:
                    return new ManhattanLayout(GroundLevel, NodeFactory.Unit);
                case NodeLayoutKind.RectanglePacking:
                    return new RectanglePackingNodeLayout(GroundLevel, NodeFactory.Unit);
                case NodeLayoutKind.EvoStreets:
                    return new EvoStreetsNodeLayout(GroundLevel, NodeFactory.Unit);
                case NodeLayoutKind.Treemap:
                    return new TreemapLayout(GroundLevel, 1000.0f * NodeFactory.Unit, 1000.0f * NodeFactory.Unit);
                ...
                case NodeLayoutKind.FromFile:
                    return new LoadedNodeLayout(GroundLevel, settings.globalCityAttributes.layoutPath.Path);
                default:
                    throw new Exception("Unhandled node layout " + settings.nodeLayoutSettings.kind);
            }
        }
            ...
            Dictionary<Node, ILayoutNode> to_layout_node)
        {
            ...
            foreach (GameObject gameObject in gameNodes)
            {
                ...
                if (node.IsLeaf())
                {
                    result.Add(new GameNode(to_layout_node, gameObject, leafNodeFactories[(int)node.kind]));
                }
                else
                {
        ...
        private int SelectStyle(Node node)
        {
            ...
            NodeFactory nodeFactory = isLeaf ? leafNodeFactories[(int)node.kind] : innerNodeFactories[(int)node.kind];
            uint numberOfStyles = nodeFactory.NumberOfStyles();
            ColoringKind coloringKind = isLeaf ? settings.leafNodeAttributesPerKind[(int)node.kind].coloringKind : settings.innerNodeAttributesPerKind[(int)node.kind].coloringKind;
        ...
        internal Vector3 GetRoof(GameObject gameNode)
        {
            ...
            if (node.IsLeaf())
            {
                return leafNodeFactories[(int)node.kind].Roof(gameNode);
            }
            else
            {
        ...
        internal Vector3 GetSize(GameObject gameNode)
        {
            ...
            if (node.IsLeaf())
            {
                return leafNodeFactories[(int)node.kind].GetSize(gameNode);
            }
            else
            {
        ...
        public void AdjustStyle(GameObject gameNode)
        {
            ...
            if (node.IsLeaf())
            {
                leafNodeFactories[(int)node.kind].SetStyle(gameNode, style);
            }
            else
            {
                innerNodeFactories[(int)node.kind].SetStyle(gameNode, style); // TODO: for some reason, the material is selected twice. once here and once somewhere earlier (i believe in NewBlock somewhere).
            }
        }
        ...
        public void AdjustScaleOfLeaf(GameObject gameNode)
        {
            ...
            if (node.IsLeaf())
            {
                ...
                if (settings.nodeLayoutSettings.kind == NodeLayoutKind.Treemap)
                {
                    ...
                    leafNodeFactories[(int)node.kind].SetWidth(gameNode, NodeFactory.Unit * widthOfSquare);
                    leafNodeFactories[(int)node.kind].SetDepth(gameNode, NodeFactory.Unit * widthOfSquare);
                    leafNodeFactories[(int)node.kind].SetHeight(gameNode, NodeFactory.Unit * scale.y);
                }
                else
                {
        ...
        protected void DrawInnerNodes(Dictionary<Node, GameObject> nodeMap, IList<Node> nodes)
        {
            for (int i = 0; i < nodes.Count; i++)
            {
                ...
                if (!nodes[i].IsLeaf())
                {
                    GameObject innerGameObject = DrawInnerNode(nodes[i]);
        ...
        public GameObject DrawInnerNode(Node node)
        {
            ...
            int style = SelectStyle(node);
            GameObject result = innerNodeFactories[(int)node.kind].NewBlock(style, node.Level);
        ...
        public void ComputeBoundingBox(ICollection<ILayoutNode> layoutNodes, out Vector2 leftLowerCorner, out Vector2 rightUpperCorner)
        {
