...
namespace SEE.Layout.NodeLayouts
{
  ...
  public abstract class NodeLayout
  {
    ...
    //protected virtual LayoutAnchor Anchor => LayoutAnchor.Center;
         ...
         Vector2 rectangle)
    {
      ...
      //MoveTo(layout.Values, new Vector3(1,1,1), box);
      var z = centerPosition.z + rectangle.y /2 ;
      //MoveTo(layout.Values, new Vector3(centerPosition.x, centerPosition.y * 1.001f, centerPosition.z), box);
      MoveTo(layout.Values, new Vector3(centerPosition.x, centerPosition.y, centerPosition.z), box);
    ...
    Vector2 rectangle)
    {
      if (Anchor == LayoutAnchor.Center || layout.Count == 0)
        return;

      Box box = Bounding3DBox(layout.Values);

      Vector3 target = centerPosition;

      switch (Anchor)
      {
        case LayoutAnchor.TopLeft:
          target.x = centerPosition.x - rectangle.x / 2f + box.Width / 2f;
          target.z = centerPosition.z + rectangle.y / 2f - box.Depth / 2f;
          break;
      ...
      // Move layout so bounding box center aligns with target
      Vector3 offset = target - box.Center;
      offset.y = 0f; // do not disturb stacking

      foreach (NodeTransform t in layout.Values)
      {
    ...
    /// <param name="prefix">A prefix to be prepended to the gameObject name.</param>
    static void Draw(Dictionary<ILayoutNode, NodeTransform> layout, string prefix, Vector3 centerPosition)
    {
      foreach (KeyValuePair<ILayoutNode, NodeTransform> entry in layout)
      {
        ...
        go.transform.localScale = new Vector3(transform.Scale.x, 0.01f, transform.Scale.z);
        UnityEngine.Object.Destroy(GameObject.Find(prefix + "+" + node.ID));
      }
    }
  ...
  {
    Center,      // current behavior (default)
    TopLeft,
    TopRight,
    BottomLeft,
    BottomRight
