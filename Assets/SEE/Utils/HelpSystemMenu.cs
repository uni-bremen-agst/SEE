...
public class HelpSystemMenu : MonoBehaviour
{
    ...
    public void CreateStartMenu(GameObject attachTo = null)
    {
        ...
        Debug.Log(navigation.MenuShown);



        // Entry´s for main Menu - title has to be unique!
        CreateNewNestedRefEntry("Navigation", "All Use-Cases in context of simple navigation in SEE", Color.gray, "Materials/Notification/info", mainMenu, navigation, mainMenu);
        CreateNewNestedRefEntry("Architecture", "All Use-Cases in context of Architecture-comparision", Color.gray, "Materials/Notification/info", mainMenu, architecture, mainMenu);
        CreateNewNestedRefEntry("Evolution", "All Use-Cases in context of Software-Evolution", Color.gray, "Materials/Notification/info", mainMenu, evolution, mainMenu);
        CreateNewNestedRefEntry("Debugging", "All Use-Cases in context of Debugging", Color.gray, "Materials/Notification/info", mainMenu, debugging, mainMenu);
        CreateNewNestedRefEntry("Quality", "All Use-Cases in context of Software-Quality", Color.gray, "Materials/Notification/info", mainMenu, quality, mainMenu);
    }
    ...
    /// <param name="menuToAdd">the menu to which this entry should be added</param>
    /// <param name="active">true, if the entry should be active, else false</param>
    public void CreateNewNestedRefEntry(string title, string description, Color color, string iconPath,
        SimpleMenu sourceMenu, NestedMenu targetMenu, SimpleMenu menuToAdd, bool active = true)
    {
        ToggleMenuEntry newEntry = new ToggleMenuEntry(
          active: active,
          entryAction: () => SwitchMenu(sourceMenu, targetMenu),
    ...
    /// <param name="activeMenu">The menu which should be closed</param>
    /// <param name="inactiveMenu">The menu which should be opened</param>
    public void SwitchMenu(SimpleMenu activeMenu, NestedMenu inactiveMenu)
    {
        activeMenu.ToggleMenu();
        inactiveMenu.ToggleMenu();
    }
    ...
    /// <param name="iconPath">the path to the icon for the new menu</param>
    /// <returns>the new SimpleMenu</returns>
    public NestedMenu CreateNewNestedMenu(GameObject objectToAddMenu, string title, string description, string iconPath, SimpleMenu parent)
    {
        NestedMenu simpleMenu = objectToAddMenu.AddComponent<NestedMenu>();
        simpleMenu.Icon = Resources.Load<Sprite>(iconPath);
        simpleMenu.Title = title;
        simpleMenu.Description = description;
        simpleMenu.parent = parent;
