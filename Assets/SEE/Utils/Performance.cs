...
namespace SEE.Utils
{
  ...
  public class Performance
  {
    ...
    public void End(bool print = false)
    {
      ...
      if (print)
      {
        Debug.Log($"Action {action} finished in {GetElapsedTime(totalTimeInMilliSeconds)} [h:m:s:ms] elapsed time).\n");
      }
    }
    ...
    public string End()
    {
      ...
      TimeSpan ts = stopWatch.Elapsed;
      totalTimeInMilliSeconds = ts.TotalMilliseconds;
      //Debug.Log($"Action {action} finished in {GetElapsedTime(this.totalTimeInMilliSeconds)} [h:m:s:ms] elapsed time).\n");
      return GetElapsedTime(totalTimeInMilliSeconds);
    }
    ...
    /// </summary>
    /// <returns>Elapsed time.</returns>
    public static string GetElapsedTime(double totalTimeInMilliSeconds)
    {
      TimeSpan ts = TimeSpan.FromMilliseconds(totalTimeInMilliSeconds);
      return $"{ts.Hours:00}:{ts.Minutes:00}:{ts.Seconds:00}.{ts.Milliseconds / 10:00}";
    }
