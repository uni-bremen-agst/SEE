            ...
            if (!IsRequester())
            {
                ...
                if (gameObject != null)
                {
                    if (gameObject.HasNodeRef())
                    {
                        ...
                        PlayerSettings.GetPlayerSettings().StartCoroutine(DeletionAnimation.MoveToGarbage(gameObject.AllAncestors()));
                        Portal.SetInfinitePortal(gameObject);
                    }
                    else if (gameObject.HasEdgeRef())
                    {
                        PlayerSettings.GetPlayerSettings().StartCoroutine(DeletionAnimation.MoveToGarbage(new List<GameObject> {gameObject}));
